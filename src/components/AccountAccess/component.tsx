...
import { sendMagicLink, signInWithApple, signInWithGoogle } from '../../services/supabaseClient';
...
import { AppleMark, GoogleMark, AuthLegal } from '../AccessGate/authBits';
...
export const AccountAccess: React.FC = () => {
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      setMessage('Could not start Apple sign-in. Please try email instead.');
      setIsSubmitting(false);
    }
  };
        ...
        <section className="grid flex-1 items-center gap-6 py-8 md:grid-cols-[1fr_400px] md:py-10">
          ...
          <div className="rounded-large border border-border-default bg-surface-primary p-4 shadow-card sm:p-5">
            ...
            {(pageState === 'email-entry' || pageState === 'magic-link-sent') && (
              <div className="space-y-3">
                <h2 className="text-[18px] font-bold text-text-primary">Save your planner</h2>
                <p className="text-[14px] leading-6 text-text-secondary">
                  Continue to save your progress and pick up your planner on any device.
                </p>
                <button
                  type="button"
                ...
                <form onSubmit={handleMagicLink} className="space-y-3">
                  <label className="block text-[12px] font-bold uppercase tracking-[0.04em] text-text-primary" htmlFor="account-email">Continue with email</label>
                  <input
                    id="account-email"
                    value={email}
